...
public class GoingDown {
    ...
    public static void main(String[] args) throws SQLException, NoSuchAlgorithmException {
        ...
        if(!GoingDown.alreadyExistingUser(email))
        {GoingDown.addToDatabase(email, password);
        ...
        if(!GoingDown.alreadyExistingUser(email2))
        {GoingDown.addToDatabase(email2, password2);
            System.out.println("We have a new player");
        }
        String email3 = "c2";
        String password3 = "admin";
        if(!GoingDown.alreadyExistingUser(email3))
        {GoingDown.addToDatabase(email3, password3);
            System.out.println("We have a new player");
        }
        String email4 = "c3";
        String password4 = "admin";
        if(!GoingDown.alreadyExistingUser(email4))
        {GoingDown.addToDatabase(email4, password4);
            System.out.println("We have a new player");
        }

    }

    private static byte[] hashing(String password) throws NoSuchAlgorithmException {
    ...
    public static void addToDatabase(String email, String rawPassword)throws SQLException, NoSuchAlgorithmException {
        Connection con = Database.getConnection();
        try{PreparedStatement pstmt = con.prepareStatement("insert into users (email, password)" + "values(?, ?)");
            String password = bytesToHex(GoingDown.hashing(rawPassword));
    ...
    public static void changePassword(String email, String newRawPassword) throws NoSuchAlgorithmException, SQLException {
        String newPassword = bytesToHex(GoingDown.hashing(newRawPassword));
        Connection con = Database.getConnection();
