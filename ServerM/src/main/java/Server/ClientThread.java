...
class ClientThread extends Thread {
    ...
    public void run() {
        try {
            ...
            while (running) {
                ...
                System.out.println("Server.Server received the request... " + request);
    ...
    private void changePassword(String request) throws SQLException, NoSuchAlgorithmException, IOException {
        String[] Pass = request.split(" ");
        boolean succes = DatabaseFunctions.verifyCredentials(email, Pass[1]);
        if(!succes) {
            sendMyMessage(failedMessage);
        }
        DatabaseFunctions.changePassword(this.email, Pass[2]);
        if(succes){
            sendMyMessage(succesMessage);
        }
    }
    ...
    private void stopServer() throws IOException {
        String answer = "Server.Server Stopped!";
        out.println(answer);
        out.flush();
        serverSocket.close();
    }
